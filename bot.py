...
# call in the code below works unchanged.

class _Cursor:
“”“Wraps a libsql cursor to look like an aiosqlite cursor.”””
def **init**(self, cur):
self._cur = cur
...
class _Conn:
“”“Async wrapper around a synchronous libsql connection.”””
def **init**(self, conn):
self._conn = conn
...
class _TursoDB:
“””
Drop-in replacement for `aiosqlite.connect(DB)`.
Usage:  async with aiosqlite.connect(DB) as db:  →  unchanged
“””
def **init**(self):
self._conn: libsql.Connection | None = None

```
def _get_conn(self) -> libsql.Connection:
    if self._conn is None:
        if TURSO_URL and TURSO_TOKEN:
            self._conn = libsql.connect(TURSO_URL, auth_token=TURSO_TOKEN)
        else:
            # Fallback to local file for development (no env vars set)
            self._conn = libsql.connect("bot.db")
    return self._conn

def connect(self) -> _Conn:
    return _Conn(self._get_conn())
```
...
LINE_H  = 20
TOP_H   = PADDING + AVA + 14           # header area
...
# accent from role color or Discord blurple
...
# card background with rounded feel (draw rectangle slightly inset)
MARGIN = 12
...
# accent stripe on left edge of card
draw.rounded_rectangle(
...
# thin accent ring around avatar
...
def **init**(self):
super().**init**(command_prefix=get_prefix, intents=intents, help_command=None)
self.join_tracker: dict[int, list] = {}

```
async def setup_hook(self):
    await init_db()
    # Register all app_commands.Group instances
...
super().**init**(placeholder=“Choose a category to configure…”, options=options)
...
kick_t = int(await get_setting(guild.id, ‘warn_kick_threshold’) or 3)
ban_t  = int(await get_setting(guild.id, ‘warn_ban_threshold’)  or 0)
mute_t = int(await get_setting(guild.id, ‘warn_mute_threshold’) or 0)
mute_m = int(await get_setting(guild.id, ‘warn_mute_minutes’)   or 10)
m = guild.get_member(member.id) if isinstance(member, discord.User) else member
if not m:
return None
...
raise ValueError
result = eval(expression, {”**builtins**”: {}}, {})  # nosec
...
@app_commands.default_permissions(manage_guild=True)
@app_commands.describe(
chapter_number=“Chapter number to announce”,
force=“Re-announce even if already announced before”
)
...
if not (SUPABASE_URL and SUPABASE_KEY):
await i.followup.send(“❌ Supabase not configured.”, ephemeral=True); return
ch_id = await get_setting(i.guild.id, ‘chapter_channel_id’)
if not ch_id:
...
print(f”Manual chapter fetch: {ex}”)
await i.followup.send(“❌ Couldn’t reach Supabase.”, ephemeral=True); return
if not results:
await i.followup.send(
f”❌ Chapter {chapter_number} not found in Supabase.”, ephemeral=True); return
chapter = results[0]
num     = chapter.get(“Chapter_number”)
title   = chapter.get(“title”, “Untitled”)
excerpt = chapter.get(“excerpt”, “”)
pub_at  = chapter.get(“created_at”, “”)
# Check if already announced (skip if force=True)
if not force:
async with aiosqlite.connect(DB) as db:
async with db.execute(
...
if already:
await i.followup.send(
f”⚠️ Chapter {num} was already announced. Use `force: True` to re-announce.”,
ephemeral=True); return
# Mark as announced
async with aiosqlite.connect(DB) as db:
await db.execute(
...
ch = bot.get_channel(ch_id)
if not ch:
await i.followup.send(“❌ Chapter channel not found.”, ephemeral=True); return
role_id = await get_setting(i.guild.id, ‘chapter_role_id’)
e = discord.Embed(
...
“**New Blood Trials chapter!** 🩸”)
try:
await ch.send(content=content, embed=e,
allowed_mentions=discord.AllowedMentions.all())
except discord.Forbidden:
await i.followup.send(“❌ Missing permissions to send in that channel.”,
ephemeral=True); return
await i.followup.send(
f”✅ Chapter {num} announced in {ch.mention}.”, ephemeral=True)

@bot.tree.command(name=“announcecharacter”,
description=“Manually announce a Blood Trials character”)
@app_commands.default_permissions(manage_guild=True)
@app_commands.describe(
name=“Character name (must match exactly as in Supabase)”,
force=“Re-announce even if already announced before”
)
...
if not (SUPABASE_URL and SUPABASE_KEY):
await i.followup.send(“❌ Supabase not configured.”, ephemeral=True); return
ch_id = await get_setting(i.guild.id, ‘character_channel_id’)
if not ch_id:
...
print(f”Manual character fetch: {ex}”)
await i.followup.send(“❌ Couldn’t reach Supabase.”, ephemeral=True); return
if not results:
await i.followup.send(
f”❌ No character named **{name}** found in Supabase.”, ephemeral=True); return
char      = results[0]
...
if already:
await i.followup.send(
f”⚠️ **{char_name}** was already announced. Use `force: True` to re-announce.”,
ephemeral=True); return
# Mark as announced
async with aiosqlite.connect(DB) as db:
await db.execute(
...
ch = bot.get_channel(ch_id)
if not ch:
await i.followup.send(“❌ Character channel not found.”, ephemeral=True); return
e = discord.Embed(
...
if not results:
await ctx.reply(f”❌ Chapter {chapter_number} not found.”); return
chapter = results[0]
num     = chapter.get(“Chapter_number”)
title   = chapter.get(“title”, “Untitled”)
excerpt = chapter.get(“excerpt”, “”)
...
if already:
await ctx.reply(
f”⚠️ Chapter {num} already announced. Run `?announcechapter {num} force` to override.”)
return
async with aiosqlite.connect(DB) as db:
await db.execute(
“INSERT OR IGNORE INTO announced_chapters (guild_id,chapter_number) VALUES (?,?)”,
...
ch = bot.get_channel(ch_id)
if not ch:
await ctx.reply(“❌ Chapter channel not found.”); return
role_id = await get_setting(ctx.guild.id, ‘chapter_role_id’)
e = discord.Embed(
title=f”📖 Chapter {num}: {title}”,
description=excerpt[:1024] if excerpt else “*No excerpt available.*”,
color=0xB22222, url=BOOK_LINK,
...
await ctx.reply(f”✅ Chapter {num} announced in {ch.mention}.”)
except discord.Forbidden:
...
“””?announcecharacter <name> [force]”””
do_force = args.lower().endswith(” force”)
name     = args[:-6].strip() if do_force else args.strip()
if not (SUPABASE_URL and SUPABASE_KEY):
await ctx.reply(“❌ Supabase not configured.”); return
ch_id = await get_setting(ctx.guild.id, ‘character_channel_id’)
if not ch_id:
...
if already:
await ctx.reply(
f”⚠️ **{char_name}** already announced. “
f”Run `?announcecharacter {name} force` to override.”)
return
async with aiosqlite.connect(DB) as db:
await db.execute(
“INSERT OR IGNORE INTO announced_characters (guild_id,char_name) VALUES (?,?)”,
...
ch = bot.get_channel(ch_id)
if not ch:
await ctx.reply(“❌ Character channel not found.”); return
e = discord.Embed(
title=f”🧬 New Character: {char_name}”,
description=char_desc[:1024] if char_desc else “*No description yet.*”,
color=0x8B0000, timestamp=datetime.now(timezone.utc))
...
try:
await ch.send(content=“🩸 **New Blood Trials character!**”, embed=e)
await ctx.reply(f”✅ **{char_name}** announced in {ch.mention}.”)
except discord.Forbidden:
await ctx.reply(“❌ Missing permissions to send in that channel.”)
...
description=desc[:1024] if desc else “*No description yet.*”,
color=0x8B0000, timestamp=datetime.now(timezone.utc))
if role: e.add_field(name=“Role”, value=role)
e.add_field(name=“📚 Read”, value=f”[Blood Trials]({BOOK_LINK})”)
try:
await ch.send(content=“🩸 **New Blood Trials character!**”, embed=e)
except discord.Forbidden:
...
if await _run_automod(message):
    return  # message was deleted, skip further processing
...
return  # real command handled by process_commands
async with aiosqlite.connect(DB) as db:
async with db.execute(
...
# Build context hint from recent channel history
recent = []
try:
...
async with aiosqlite.connect(DB) as db:
    await db.execute(
        ...
        (gid, payload.message_id))
    await db.commit()
sb_channel = bot.get_channel(sb_ch_id)
if not sb_channel:
    return
